...
abstract final class TemplateRenderer {
  ...
  static String _pickTemplate(String norm, FeatureOptions options) {
    ...
    // ── Retrofit API ──
    if (norm.contains('datasources/') && norm.endsWith('_api.dart')) {
      return _tplRetrofitApi;
    }
    ...
    if (norm.endsWith('_remote_datasource.dart')) {
      if (options.hasRetrofit) {
        if (options.hasMultipleDatasources) {
          return options.isList
              ? _tplRetrofitRemoteDatasourceWithMock
              : _tplRetrofitRemoteDatasourceSingleWithMock;
        }
        return options.isList
            ? _tplRetrofitRemoteDatasource
            : _tplRetrofitRemoteDatasourceSingle;
      }
...
const _tplRetrofitApi = r'''
import 'package:dio/dio.dart';
import 'package:retrofit/retrofit.dart';

import '../models/{{featureSnake}}_model.dart';

part '{{featureSnake}}_api.g.dart';

@RestApi()
abstract class {{Feature}}Api {
  factory {{Feature}}Api(Dio dio, {String? baseUrl}) = _{{Feature}}Api;

  @GET('{{featureSnake}}/')
  Future<List<{{Feature}}Model>> getAll();

  @GET('{{featureSnake}}/{id}')
  Future<{{Feature}}Model> getById(@Path('id') String id);

  @POST('{{featureSnake}}/')
  Future<{{Feature}}Model> create(@Body() Map<String, dynamic> body);
...
const _tplRetrofitRemoteDatasource = r'''
import 'package:injectable/injectable.dart';
import 'package:nexo/nexo_core.dart';

import 'i_remote_{{featureSnake}}_data_source.dart';
import '{{featureSnake}}_api.dart';
import '../models/{{featureSnake}}_model.dart';
...
class {{Feature}}RemoteDataSource extends BaseRemoteDataSource
  ...
  @override
  Future<{{Feature}}Model> create(Map<String, dynamic> body) async {
    return _api.create(body);
  }

  @override
  Future<{{Feature}}Model> update(String id, Map<String, dynamic> body) async {
...
const _tplRetrofitRemoteDatasourceSingle = r'''
import 'package:injectable/injectable.dart';
import 'package:nexo/nexo_core.dart';
...
const _tplRetrofitRemoteDatasourceWithMock = r'''
import 'package:injectable/injectable.dart';
import 'package:nexo/nexo_core.dart';
...
const _tplRetrofitRemoteDatasourceSingleWithMock = r'''
import 'package:injectable/injectable.dart';
import 'package:nexo/nexo_core.dart';
