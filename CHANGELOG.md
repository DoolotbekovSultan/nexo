...
- `nexo_core`: новые **`NexoValidators`** — готовые валидаторы форм (`requiredField`, `email`, `phone`, `url`, `number`, `minLength` / `maxLength`, `password`, композитор `compose`), совместимые с `TextFormField.validator`. Тексты ошибок берутся из каталога сообщений пакета (включая подстановку имени поля) и локализуются вместе с ним.
